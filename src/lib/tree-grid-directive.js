(function () {
        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    ...
                    "   </thead>\n" +
                    "   <tbody>\n" +
                    "     <tr title=\"{{row.branch.observation}}\" ng-click=\"user_clicks_branch(row)\" ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.uid\"\n" +
                    "       ng-class=\"'level-' + {{ row.level }} + (row.selected ? ' active':'')\" class=\"tree-grid-row\">\n" +
                    "       <td>teste<a><i ng-class=\"row.tree_icon\"\n" +
                    ...
                    "              <span  ng-if=\"!expandingProperty.cellTemplate\" class=\"indented tree-label\" ng-click=\"on_user_click(row)\">\n" +
                    "             {{row[expandingProperty.field] || row[expandingProperty]}}</span>\n" +
                    "       </td>\n" +
                    "       <td ng-repeat=\"col in colDefinitions\">\n" +
                    "         <div ng-if=\"col.cellTemplate\" compile=\"col.cellTemplate\" cell-template-scope=\"col.cellTemplateScope\"></div>\n" +
                    "         <div ng-if=\"!col.cellTemplate\">{{row[col.field]}}</div>\n" +
                    "       </td>\n" +
                    "     </tr>\n" +
                    "   </tbody>\n" +
                    " </table>\n" +
    ...
    angular
        ...
        .filter('searchFor', function () {
            ...
            function removeChildrens(item, filterString) {
            ...
            function checkItem(item, filterString, col) {
                ...
                if (item.children && item.children.length > 0)
                    return checkItemChilds(item, filterString, col);
            }

            function checkItemChilds(item, filterString, col) {
                var list = item.children;
                item.children = [];
                for (var i = 0; i < list.length; i++) {
                    var element = list[i];
                    if (col.sortingType === "number") {
                        if (element[col.field] != null &&
                            parseFloat(element[col.field]) === parseFloat(filterString)) {
                            item.children.push(element);
                            return true;
                        }
                    } else {
                        if (element[col.field] != null &&
                            element[col.field].toLowerCase().indexOf(filterString.toLowerCase()) !== -1) {
                            item.children.push(element);
                            return true;
                        }
                    }
                    if (element.children && element.children.length > 0)
                        return checkItemChilds(element, filterString, col);
                };
            }
